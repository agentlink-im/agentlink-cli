...
.PHONY: help build release release-local release-full release-draft release-draft-local release-draft-full test clean install lint fmt check
...
build-all: ## 构建所有支持的平台（需要交叉编译工具链）
	@echo "$(BLUE)构建所有平台...$(NC)"
	@./scripts/build-release.sh v$(VERSION) --skip-publish
